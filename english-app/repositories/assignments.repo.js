...
async function listForTeacher(teacherId) {
  ...
  const { rows } = await pool.query(`
    SELECT a.id, a.title, a.status, a.created_at,
           ...
           (SELECT COUNT(*) FROM eng_attempts at WHERE at.assignment_id = a.id)::int AS attempt_count,
...
async function getOne({ assignmentId }) {
  ...
  const { rows: aRows } = await pool.query(`
    SELECT id, title, status, target_classname, target_group, created_by
      FROM eng_assignments WHERE id = $1`, [assignmentId]);
  if (!aRows[0]) return null;
  const { rows: iRows } = await pool.query(`
    SELECT id, word, hint, image_url, order_index
      FROM eng_assignment_items
...
async function create({ teacherId, targetClassname, targetGroup, title, status, items }) {
  return withTransaction(async client => {
    const { rows: aRows } = await client.query(
      `INSERT INTO eng_assignments (target_classname, target_group, title, status, created_by)
...
async function updateStatus({ assignmentId, teacherId, status }) {
  ...
  const { rowCount } = await pool.query(
    `UPDATE eng_assignments SET status = $1 WHERE id = $2 AND created_by = $3`,
...
async function remove({ assignmentId, teacherId }) {
  const pool = requirePg();
  const { rowCount } = await pool.query(
    `DELETE FROM eng_assignments WHERE id = $1 AND created_by = $2`,
...
async function teacherOwns({ assignmentId, teacherId }) {
  const pool = requirePg();
  const { rows } = await pool.query(
    `SELECT 1 FROM eng_assignments WHERE id = $1 AND created_by = $2`,
    [assignmentId, teacherId]);
  return rows.length > 0;
}
...
module.exports = {
  listForTeacher, listForStudent, getOne, create, updateStatus, remove,
  studentHasAccess, teacherOwns, listGroupSummary, groupLabel,
};
