// English-module shared helpers (wrapped in IIFE so only window.EnglishApp leaks).
(function () {
  'use strict';
  ...
  async function ensureAuth(requireRole) {
    ...
    if (requireRole && me.role !== requireRole) {
      if (me.role === 'teacher') window.location.href = '/english/teacher';
      else window.location.href = '/english/student';
      return null;
    }
    return me;
  }

  window.EnglishApp = { api, getMe, toast, el, fmtDate, renderTopbar, ensureAuth };
})();
