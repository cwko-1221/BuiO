...
const ROOMS = [
  ...
  ['cloud-loft', '雲端閣樓', `an airy attic above the clouds: pale bleached birch floorboards, soft sky-blue plaster walls with a white picture rail, a large round porthole window on the back wall full of drifting white cloud`],
  ['ocean-cabin', '海洋船艙', `a ship's cabin: dark polished teak planking, painted teal wainscot below a varnished timber rail, a brass porthole on the back wall showing calm blue water, thick rope trim along the skirting`],
  ['forest-treehouse', '森林樹屋', `a treehouse interior: wide rough-sawn timber floorboards, bark-textured plank walls with ivy creeping in at the corners, a large leaf-framed opening on the back wall looking out into green canopy`],
  ['space-pod', '星際睡艙', `a sleeping pod aboard a starship: pale grey seamless composite floor, deep indigo padded wall panels with soft glowing seams, a wide viewport on the back wall full of stars and a distant nebula`],
  ['candy-workshop', '糖果工房', `a confectioner's workshop: a chequered mint and cream tiled floor, glossy pink sugar-glazed walls with a scalloped border, a rounded pastry-shop window on the back wall with a striped awning inside`],
  ['lava-den', '熔岩龍窩', `a dragon's den: dark basalt flagstones with faint ember cracks glowing between them, rough volcanic rock walls, a glowing lava seam running across the back wall behind a low stone arch`],
...
const FURNITURE_ROLES = [
  ...
  'a signature ornament that says what this room is',
];
...
for (let i = 0; i < ROOMS.length; i += 2) {
  const [idA, zhA, themeA] = ROOMS[i];
  const [idB, zhB, themeB] = ROOMS[i + 1];
...
Cells 1 to 10 all belong to one room: ${themeA}
${list(0)}

Cells 11 to 20 all belong to a different room: ${themeB}
${list(10)}
