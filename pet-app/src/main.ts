...
const MYSTERY_SHOP_PET_IDS = new Set([
...
class StudentApp {
  ...
  private renderShop(category='eggs') {
    ...
    if(category==='eggs')cards=`<article class="shop-feature"><div class="shop-egg"></div><div><span class="rarity common">${this.t('probability')}</span><h3>${this.t('randomEgg')}</h3><p>${this.locale==='zh-HK'?'隨機獲得已開放寵物之一；未擁有品種優先。':'Receive one of the released pets; unseen species are favoured.'}</p><button class="primary" data-action="buy-random-egg">${this.t('buy')} · ${this.state.catalog.egg.randomPrice} ${this.t('coins')}</button></div></article><h2>${this.t('directPet')}</h2><div class="shop-grid">${this.state.catalog.pets.map((item)=>{const owned=this.state.pets.some((pet)=>pet.speciesId===item.id);const mystery=MYSTERY_SHOP_PET_IDS.has(item.id);return `<article class="shop-card ${mystery?'mystery-pet':''}"><img src="${item.art[0]}" alt="" loading="lazy"><span class="rarity ${item.rarity}">${item.rarity}</span><h3>${mystery?'???':escapeHtml(this.petName(item,1))}</h3><button data-action="buy-direct-egg" data-id="${item.id}" ${owned?'disabled':''}>${owned?this.t('owned'):`${this.t('buy')} · ${item.directPrice ?? (item.rarity==='common'?1200:2200)}`}</button></article>`;}).join('')}</div>`;
