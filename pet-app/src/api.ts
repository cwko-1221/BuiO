...
export const api = {
  ...
  setLoadout: (petId: string, skillIds: string[]) => request<any>(`/api/pet/pets/${encodeURIComponent(petId)}/loadout`, { method: 'PUT', body: JSON.stringify({ skillIds }) }),
  ...
  startRun: (mapId: string) => request<any>('/api/pet/runs/start', { method: 'POST', body: JSON.stringify({ mapId }) }),
  completeRun: (body: { runId: string; success: boolean; badgeFound: boolean }) => request<any>('/api/pet/runs/complete', { method: 'POST', body: JSON.stringify(body) }),
